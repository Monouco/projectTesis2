...
double distanceL (double x1[], double x2[],double L) 
...
double fun(double x[])
   {
    ...
    switch (Pr)
    {
      ...
      case 4: // Rastrigin. Minimum value 0. Solution (0,0 ...0)
		  {k = 10;
        f = 0;
        for ( d = 0; d < D; d++ )
        {
...
f = s1+s2*s2+s2*s2*s2*s2;		 
 ...
 case 11: //Cigar [-10,10]  f(0,0,0,...,0)=0
...
case 15: // Axis parallel hyperellipsoid
{
        f = 0;
        for ( d = 0; d < D; d++ )
        {
...
case 18: //step function [-100, 100] f(-0.5<=x<=0.5)=0
 {
...
f = (3.14/D)*(10*sin(3.14*(1+0.25*(x[0]+1)))*sin(3.14*(1+0.25*(x[0]+1)))+s+((1+0.25*(x[D-1]+1))-1)*((1+0.25*(x[D-1]+1))-1));
  ...
  //===========================================================
 case 24: //Levi montalvo 2 
{
		                                                        
double s =0;
for(d=0;d<D-1;d++)
          s+=(x[d]-1)*(x[d]-1)*(1+sin(3*3.14*x[d+1])*sin(3*3.14*x[d+1]));
         f = 0.1*(sin(3*3.14*x[0])*sin(3*3.14*x[0])
            	+s+(x[D-1]-1)*(x[D-1]-1)*(1+sin(2*3.14*x[D-1])*sin(2*3.14*x[D-1])));
 		 break;
		 }
...
f+=pow((x[d]-d-1),2);
	   ...
	   case 26:		//Beale function [-4.5,4.5] f(3, 0.5)=0
    ...
    case 28: //Branins’s function [-5,10][0,15] f(-pi, 12.275)=0.3979		
     ...
     case 29: //Kowalik function [-5,5] f(0.192833, 0.190836, 0.123117, 0.135766)=0.000307486
    ...
    case 30:		//// 2D Tripod function [-100,100] f(0, 50)=-50
     ...
     case 31:	//Shifted CEC 2005 Rosenbrock F6  [-100, 100], solution point is O + (1; : : : ; 1) where f = 390.
	 {
  	...
  	case 32: // /// Shifted Parabola/Sphere (CEC 2005 benchmark)	x[-100,100] , Global optimum: x* = offset  f(x) = f_bias = - 450
   	{
    ...
    case 33: //Shifted CEC 2005  Rastrigin  x[-5,5] , Global optimum x* = offset , f(x*) = f_bias = - 330
   {
...
case 34: //Shifted CEC 2005 Schwefel [-100,100], Global optimum x* = offset , f(x*) = f_bias = - 450
  {
 ...
 case 35: //Shifted CEC 2005 Griewank. WARNING: in the CEC 2005 benchmark it is rotated
  {
 ...
 case 36:  // Shifted Ackley (CEC 2005) [-32,32], Global optimum x* = offset , f(x*) = f_bias = - 140
  {
 ...
 /*case 37:		// Compression spring  [1,...,70],[0.6,3],[0.207,0.5] f(7; 1:386599591; 0:292) = 2:6254214578.
			
       {
           	double s11, s12, s21, s22;		
			x1 = (int)x[0] ; 
		    x2 = x[1];  
		    x3 = double((int)(x[2]*1000))/1000.0;
		    if (x2==0)
		       x2=1.8;
		    double sw=1.25;
		    double spm=6;
		    ...
		    double Fmax=1000;
		    double S=189000;
		    double cf=1+(0.75*x3/(x2-x3))+0.615*(x3/x2);
		    double k=11.5*1000000*pow(x3,4)/(8*x1*pow(x2,3));
		    double lf=(Fmax/k)+(1.05*(x1+2)*x3);
		    double sp=Fp/k;
	     
	       double g1=(8*cf*Fmax*x2)/(3.14*pow(x3,3))-S;
	       double g2=lf-lmax;
	       double g3=sp-spm;
	       double g4=sw-(Fmax-Fp)/k;
	       
	       if(g1<=0 && g2<=0 &&g3<=0 &&g4<=0)
	               f=fabs((pi*pi*(x2*pow(x3,2)*(x1+2))/4));
           else
                   f=9999;
          
		break;
    }*/
    // Case 38: Gear Train Problem
   case 39:		// Goldstein-Price function  [-2,2] f(0, -1) = 3.
 	 ...
 	 case 40:	//Six-hump camel back function  [-5 < x < 5] f(-0.0898,0.7126) = -1.0316.
			
       {
        ...
        case 41:	//Easom's function  -10<=x(i)<=10, i=1:2. f(x1,x2)=-1; (x1,x2)=(pi,pi).
...
///////////////////////////////////////////
 case 42:	////Dekkers and Aarts Problem (DA)  -20<=x(i)<=20, i=1:2. f(0,15),f(0,-15)=-24777; 
      ...
      case 45:	 //Meyer and Roth Problem (MR) (Wolfe, 1978)  -10<=x<=10, f(ð3.13; 15.16; 0.78)=0.4*10^(-4); 
       ...
       case 46:	 //Shubert Problem (SBT)  -10<=x<=10, f((7.0835, 4.8580)=-186.7309;
      ...
      /*case 55 :	 // Shekel Foxholes Functions; 
			
       {
               ...
               x2=x[1];
               x3=x[2];
               double Arry[][]={{-32,-16,0,16,32,-32,-16,0,16,32,-32,-16,0,16,32,-32,-16,0,16,32,-32,-16,0,16,32},{-32,-32,-32,-32,-32,-16,-16,-16,-16,-16,0,0,0,0,0,16,16,16,16,16,32,32,32,32,32}};
               for(int j=0;j<D;j++)
               {
                  s1=0;
                  for(int i=0;i<2;i++)
			...
			
       {
               ...
               double s3=0;
               double s4=0;
               double CArry[][]={{4.0,1.0,8.0,6.0,3.0,2.0,5.0,8.0,6.0,7.0},{4.0,1.0,8.0,6.0,7.0,9.0,5.0,1.0,2.0,3.6},{4.0,1.0,8.0,6.0,3.0,2.0,3.0,8.0,6.0,7.0},{4.0,1.0,8.0,6.0,7.0,9.0,3.0,1.0,2.0,3.6}};
               double BArry[]={0.1,0.2,0.2,0.4,0.4,0.6,0.3,0.7,0.5,0.5};
               for(int j=0;j<D;j++)
               {
                  s1=0;
                  for(int i=0;i<4;i++)
       ...
       case 59 :	 // Hartmann Functions 3 dim; 
			
       {
               ...
               double s3=0;
               double s4=0;
               double AArry[][]={{3.0,10,30},{0.1,10,35},{3.0,10,30},{0.1,10,35}};
               double PArry[][]={{6890,1170,2673},{4699,4387,7470},{1091,8732,5547},{381,5743,8828}};
               double LArry[]={1,1.2,3,3.2};
               for(int i=0;i<4;i++)
               {
                  s1=0;
                  for(int j=0;j<3;j++)
                  {
                      ...
                      s1=s1+AArry[i][j]*(pow(x[j]-0.0001*PArry[i][j]),2);
                 }
                 s2=s2+LArry[i]*exp(-s1);
                 
               }
                 f=-s2;
                break;
       }
case 60 :	 // Hartmann Functions 6 dim; 
			
       {
               ...
               double s3=0;
               double s4=0;
               double AArry[][]={{10,3,17,3.05,1.7,8},{0.05,10,17,0.1,8,14},{3,3.5,1.7,10,17,8},{17,8,0.05,10,0.1,14}};
               double PArry[][]={{1312,1696,5569,124,8283,5886},{2329,4135,8307,3736,1004,9991},{2348,1451,3522,2883,3047,6650},{4047,8828,8732,5743,1091,381}};
               double LArry[]={1,1.2,3,3.2};
               for(int i=0;i<4;i++)
               {
                  s1=0;
...
void initilize_params(int Pr)
{
 ...
 if (Pr==0)// Parabola (Sphere)
        ...
        if (Pr==4)//// Rastrigin. Minimum value 0. Solution (0,0 ...0)
        {
           D=30;
           obj_val=0;
           acc_err=1.0e-5;
        ...
        if (Pr==11)//Cigar [-10,10]  f(0,0,0,...,0)=0
        {
           D=30;
           obj_val=0;
           acc_err=1.0e-5;
        ...
        if (Pr==15)// Axis parallel hyperellipsoid
        {
           D=30;
           obj_val=0;
           acc_err=1.0e-5;
        ...
        if (Pr==18)//step function [-100, 100] f(-0.5<=x<=0.5)=0
        {
           D=30;
           obj_val=0;
           acc_err=1.0e-5;
        ...
        if (Pr==26)//Beale function [-4.5,4.5] f(3, 0.5)=0
        {
           D=2;
           obj_val=0;
           acc_err=1.0e-5;
        ...
        if (Pr==28)//Branins’s function [-5,10][0,15] f(-pi, 12.275)=0.3979
        {
               D=2;
               acc_err=1.0e-5;
               obj_val=0.3979;
         ...
         if (Pr==31)//Shifted Rosenbrock F6 CEC 2005 [-100, 100], solution point is O + (1; : : : ; 1) where f = 390.
        {
           ...
           if (Pr==32)//// Shifted Parabola/Sphere (CEC 2005 benchmark)	x¸[.100,100] , Global optimum: x* = o , 1( *) 1 F x = f_bias = - 450
        {
                D=10;
               acc_err=1.0e-5;
               obj_val=-450;
      ...
      if (Pr==33)//Shifted CEC 2005  Rastrigin  x[-5,5] , Global optimum x* = offset , f(x*) = f_bias = - 330
        {
                D=10;
               acc_err=1.0e-2;
               obj_val=-330;
         ...
         if (Pr==34)//Shifted CEC 2005 Schwefel [-100,100], Global optimum x* = offset , f(x*) = f_bias = - 450
        {
               D=10;
               acc_err=1.0e-5;
               obj_val=-450;
         ...
         if (Pr==35)//Shifted CEC 2005 Griewank. WARNING: in the CEC 2005 benchmark it is rotated
        {
                D=10;
               acc_err=1.0e-5;
               obj_val=-180;
         ...
         if (Pr==36)// Shifted Ackley (CEC 2005) [-32,32], Global optimum x* = offset , f(x*) = f_bias = - 140
        {
                D=10;
               acc_err=1.0e-5;
               obj_val=-140;
        ...
        /// if (Pr=38) Gear Train Problem
         if (Pr==39)// 	// Goldstein-Price function  [-2,2] f(0, -1) = 3.
        {
                D=2;
               acc_err=1.0e-14;
               obj_val=3;
         ...
         if (Pr==40)// 	//Six-hump camel back function  [-5 < x < 5] f(-0.0898,0.7126) = -1.0316.
        {
                D=2;
               acc_err=1.0e-5;
               obj_val=-1.0316;
          ...
          if (Pr==41)// 	//Easom's function  -10<=x(i)<=10, i=1:2. f(x1,x2)=-1; (x1,x2)=(pi,pi).
        {
               D=2;
               acc_err=1.0e-13;
               obj_val=-1;
        ...
        if (Pr==42)// 	//Dekkers and Aarts Problem (DA)  -20<=x(i)<=20, i=1:2. f(0,15),f(0,-15)=-24777; 
        {
               D=2;
               acc_err=0.5;
               obj_val=-24777;
         ...
         if (Pr==44)// 	McCormick Problem (MC)   -1.5<=x1<=4, -3<=x2<=3,f(-0.547, -1.547)==-1.9133; 
        {
         ...
         if (Pr==45)//Meyer and Roth Problem (MR) (Wolfe, 1978)  -10<=x<=10, f(ð3.13; 15.16; 0.78)=0.4*10^(-4); 
        {
               D=3;
               acc_err=1.95e-3;
               obj_val=-0.4e-4;
        ...
        if (Pr==46)//Shubert Problem (SBT)  -10<=x<=10, f((7.0835, 4.8580)=-186.7309; 
        {
               D=2;
               acc_err=1.0e-5;
               obj_val=-186.7309;
          ...
          if (Pr==54)  //Schwefel 
      {
			...
			
       {
        ...
        if (Pr==59)	 // Hartmann Functions 3 dim; 
       ...
       if (Pr==60)	 // Hartmann Functions 6 dim; 
